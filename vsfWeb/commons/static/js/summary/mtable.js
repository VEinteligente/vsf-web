$(document).ready(function(){
	...
	.done(function(data){
		...
		$.each(dataJson.events,function(index,value){

			//Site that has been measured
			site = value.target.site;

			//url from the measurement
			m_url = value.target.url;		


			//Json from the row that is inserted in the table
			raw_data = JSON.stringify(value.flags[idcount]);
			
			$.each(value.flags, function (index, value){


				//country of the measurement
				country = value.probe.country;
				//city of the measurement
				city = value.probe.city;
				//state or region of the measurement
				state = value.probe.region;
				//isp of the measurement
				isp = value.probe.isp;
				//type of measurement
				type = value.type_med;
				...
				$("#dataTableM").append('<div class="row" id="mRow"><div class="col-xs-3 mData" id="mDataSite">'+site+'</div><div class="col-xs-2 mData">'+m_url+'</div><div class="col-xs-1 mData" id="mDataDate">'+date.getDate()+'/'+month_number[date.getMonth()]+'/'+date.getFullYear()+'</div><div class="col-xs-1 mData" id="mDataCountry">'+country+'</div><div class="col-xs-1 mData" id="mDataRegion">'+state+'</div><div class="col-xs-1 mData" id="mDataCity">'+city+'</div><div class="col-xs-1 mData" id="mDataIsp">'+isp+'</div><div class="col-xs-1 mData" id="mDataJson"><a data-id="'+value.id+'" data-json=\''+raw_data+'\' id="modalbutton" data-toggle="modal" data-target="#myModal">.Json</a></div><div class="col-xs-1 mData" id="mDataType">'+type+'</div></div>');
				idcount= idcount +1;
...
$(document).on("click", "#modalbutton", function () {
	...
	$("#measurementTitle").html('Measurement ID <span id="mModalID">'+mid+'</span>');
});
