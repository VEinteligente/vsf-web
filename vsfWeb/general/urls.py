...
urlpatterns = [
    ...
    url(r'^advanced-list-cases/title=(?P<title>(\S|\W)*)&category=(?P<category>(\S|\W)*)&start_date=(?P<s_year>\d*)-(?P<s_month>\d*)-(?P<s_day>\d*)&end_date=(?P<e_year>\d*)-(?P<e_month>\d*)-(?P<e_day>\d*)&region=(?P<region>(\S|\W)*)&domain=(?P<domain>(\S|\W)*)&site=(?P<site>(\S|\W)*)&isp=(?P<isp>(\S|\W)*)/$',CaseListAdvanced.as_view(), name="CaseListAdvanced"),
