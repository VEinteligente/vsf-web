$(document)
		.ready(
				function() {
										...
										$
												.each(
														...
														function(index, value) {
															...
															// in the table
															raw_data = JSON
																	.stringify(dataJson.results[idcount]);
															...
															$("#dataTableM")
																	.append(
																			'<div class="row" id="mRow"><div class="col-xs-3 mData">'
																					+ value.target.site
																					+ '</div><div class="col-xs-2 mData">'
																					+ value.target.url
																					+ '</div><div class="col-xs-1 mData" style="font-size:12px">'
																					+ date
																							.getDate()
																					+ '/'
																					+ month_number[date
																					...
																					+ '</div><div class="col-xs-1 mData">'
																					+ value.isp
																					+ '</div><div class="col-xs-1 mData"><a data-id="'
																					+ value.id
																					+ '" data-json=\''
																					+ raw_data
																					+ '\' id="modalbutton" data-toggle="modal" data-target="#myModal">.Json</a></div><div class="col-xs-1 mData">'
																					+ value.type_med
																					+ '</div></div>');
															idcount = idcount + 1;
					...
					// Function to give the modal the Json file on click with
					// the format we like to

					$(document).on("click", "#modalbutton", function() {
						// Here we gran the Json from de data-json field
						var raw_json = $(this).data('json');

						// Id from the measurement that goes in the title of the
						// modal
